...
export default async function SettlementsPage({
  ...
  // 1. 매출월 옵션
  const { data: monthsRes } = await supabase
    .from("sales")
    .select("sales_month")
    .order("sales_month", { ascending: false });
  const allMonths = Array.from(
    new Set((monthsRes ?? []).map((r) => r.sales_month)),
  );
  const sourceMonth = source ?? allMonths[0] ?? "";

  // 2. 선택된 매출월의 "수금완료 + 미정산" 매출 조회
  ...
  if (sourceMonth) {
    const { data: pending, error: pErr } = await supabase
      ...
      .select(
        `
        id, closing_date, customer, product_code, product_name,
        quantity, supply_amount, commission_amount,
        rep_id, rep_name_raw,
        profiles:rep_id ( name )
      `,
      )
      .eq("sales_month", sourceMonth)
    ...
    pendingGroups = Array.from(map.values()).sort((a, b) => {
